...
function validateAnswerRelevance(answerText, questionText) {
  ...
  if (lowerQuestion.includes('api') || lowerQuestion.includes('rest')) {
    questionConcepts.push('api', 'rest', 'endpoint', 'route', 'http', 'method', 'get', 'post', 'put', 'delete', 'fastapi', 'axios', 'fetch', 'request', 'response');
    if (lowerQuestion.includes('frontend') || lowerQuestion.includes('backend')) {
      questionConcepts.push('frontend', 'backend', 'client', 'server');
    }
  }
  
  // Database questions
  if (lowerQuestion.includes('database') || lowerQuestion.includes('sql') || lowerQuestion.includes('schema')) {
    questionConcepts.push('database', 'sql', 'schema', 'table', 'query', 'index', 'foreign key', 'relationship', 'normalization', 'postgresql', 'mysql', 'mongodb');
  }
  
  // Error Handling questions
  if (lowerQuestion.includes('error') || lowerQuestion.includes('invalid') || lowerQuestion.includes('exception')) {
    questionConcepts.push('error', 'exception', 'validation', 'try', 'catch', 'handle', 'status code', '400', '500', 'input', 'pydantic', 'logging');
  }
  
  // Scalability questions
  if (lowerQuestion.includes('scalability') || lowerQuestion.includes('performance')) {
    questionConcepts.push('scalability', 'performance', 'load', 'scale', 'cache', 'index', 'optimization', 'horizontal', 'vertical', 'kubernetes', 'redis', 'load balancer');
  }
  
  // System Design questions
  if (lowerQuestion.includes('design') || lowerQuestion.includes('architecture')) {
    questionConcepts.push('design', 'architecture', 'component', 'service', 'database', 'cache', 'load balancer', 'microservices');
  }
  ...
  // Check if answer is completely off-topic (no matching concepts)
  if (questionConcepts.length > 0 && matchedConcepts === 0) {
    // Check if it's a behavioral story that doesn't match
    const isBehavioralStory = lowerAnswer.includes('pressure') || 
    ...
    if (isBehavioralStory && !lowerQuestion.includes('behavioral') && !lowerQuestion.includes('pressure') && !lowerQuestion.includes('conflict')) {
      return {
        valid: false,
    ...
    // Check if it's a system design answer for a different question
    const isSystemDesignAnswer = lowerAnswer.includes('microservices') || 
    ...
    if (isSystemDesignAnswer && !lowerQuestion.includes('design') && !lowerQuestion.includes('architecture') && !lowerQuestion.includes('scalability')) {
      return {
        valid: false,
  ...
  // For API questions, check if answer includes API-related content
  ...
  if ((lowerQuestion.includes('scalability') || lowerQuestion.includes('performance')) && matchedConcepts < 2) {
    return {
      valid: false,
  ...
  // Check if answer length is appropriate for the question type
  const wordCount = answerText.split(/\s+/).length;
  if (lowerQuestion.includes('design') || lowerQuestion.includes('architecture') || lowerQuestion.includes('scalability')) {
    if (wordCount < 20) {
      return {
        valid: false,
...
function validateTechnicalAnswer(answerText, questionText) {
  ...
  if (cleanText.length > 10) {
    ...
    if (maxConsecutiveConsonants > 8) {
      return { valid: false, reason: "Your answer appears to contain random text. Please provide a meaningful technical response." };
    }
  }
  ...
  if (cleanText.length > 5 && vowelCount === 0) {
    return { valid: false, reason: "Your answer doesn't appear to contain meaningful words. Please provide a proper technical response." };
  }
  
  // 3. Check minimum word count (increased to 10)
  if (wordCount < 10) {
    return { valid: false, reason: `Please provide a more detailed answer. Your response should be at least 10 words (${wordCount} words provided).` };
  }
  ...
  const techKeywords = [
    ...
    'html', 'css', 'rest', 'api', 'graphql', 'websocket', 'http', 'json', 'xml',
    'sql', 'mysql', 'postgresql', 'mongodb', 'redis', 'oracle', 'database', 'schema',
    'table', 'query', 'index', 'join', 'transaction', 'acid', 'nosql', 'foreign key',
    'docker', 'kubernetes', 'aws', 'azure', 'gcp', 'ci/cd', 'jenkins', 'git', 'github',
    'microservices', 'monolithic', 'serverless', 'event-driven', 'message queue',
    'rabbitmq', 'kafka', 'load balancer', 'cache', 'cdn',
    'authentication', 'authorization', 'jwt', 'oauth', 'ssl', 'tls', 'encryption',
    'hashing', 'salt', 'csrf', 'xss', 'sql injection',
    'unit test', 'integration test', 'e2e', 'jest', 'pytest', 'junit', 'mock', 'stub',
    'algorithm', 'data structure', 'oop', 'functional', 'design pattern', 'solid',
    'agile', 'scrum', 'kanban', 'monitoring', 'logging', 'deployment',
    'frontend', 'backend', 'fullstack', 'server', 'client', 'database', 'llm',
    'ai', 'machine learning', 'deep learning', 'nlp', 'neural network',
    'architecture', 'scalability', 'reliability', 'availability', 'consistency',
    'partition', 'replication', 'sharding', 'caching', 'load balancing',
    'software', 'development', 'programming', 'framework', 'library', 'tool',
    'experience', 'years', 'worked', 'project', 'team', 'collaboration',
    'graduate', 'degree', 'computer science', 'engineering', 'information technology',
    'developer', 'engineer', 'programmer', 'coder', 'fullstack', 'devops',
    'axios', 'fetch', 'endpoint', 'route', 'postman'
  ];
  ...
  // Check if this is the introduction question
  const isIntroductionQuestion = questionText.toLowerCase().includes('introduce') || 
                                 questionText.toLowerCase().includes('background') ||
                                 questionText.toLowerCase().includes('yourself');
  
  // For introduction questions, need at least 2 technical keywords
  ...
  // For introduction questions, check for technical context words
  if (isIntroductionQuestion) {
    const hasTechnicalContext = lowerAnswer.includes('experience') || 
  ...
  // 5. Check if answer is relevant to the specific question
  const relevanceCheck = validateAnswerRelevance(answerText, questionText);
  if (!relevanceCheck.valid) {
    return relevanceCheck;
  }
  
  // 6. For other technical questions, need at least 1 keyword
  const technicalQuestionIndicators = [
    'technical', 'database', 'api', 'rest', 'backend', 'design', 
    'architecture', 'error', 'security', 'authentication', 'test',
    'deploy', 'framework', 'library', 'tool', 'language', 'system',
    'algorithm', 'data structure', 'cloud', 'devops', 'scalability',
    'code', 'program', 'develop', 'build'
  ];
  
  const isTechnicalQuestion = technicalQuestionIndicators.some(indicator => 
...
function calculateAnswerScore(answerText, questionText) {
  ...
  // 1. Length-based scoring (max 35 points)
  ...
  // 2. Technical keyword detection (max 35 points)
  const techKeywords = [
    'java', 'python', 'javascript', 'typescript', 'c++', 'c#', 'ruby', 'go', 'rust',
    'react', 'angular', 'vue', 'node', 'express', 'django', 'flask', 'fastapi', 'spring',
    'html', 'css', 'rest', 'api', 'graphql', 'websocket', 'http', 'json', 'xml',
    ...
    'unit test', 'integration test', 'e2e', 'jest', 'pytest', 'junit', 'mock', 'stub',
    'algorithm', 'data structure', 'oop', 'functional', 'design pattern', 'solid',
    'agile', 'scrum', 'kanban', 'monitoring', 'logging', 'deployment', 'llm',
    'ai', 'machine learning', 'deep learning', 'nlp', 'architecture',
    'scalability', 'reliability', 'availability', 'consistency',
    'axios', 'fetch', 'postman', 'endpoint', 'pydantic'
  ];
  ...
  // 4. Technical depth (max 15 points)
  let depthScore = 0;
  const depthIndicators = [
    'because', 'therefore', 'however', 'although', 'specifically',
    'for example', 'such as', 'including', 'implement', 'designed', 
    'developed', 'created', 'built', 'challenge', 'solution', 
    'approach', 'method', 'strategy', 'efficient', 'performance', 
    'scalability', 'reliability', 'architecture', 'design',
    'implemented', 'organized', 'configured', 'tested', 'deployed'
  ];
...
function populateQuestionTypes() {
  ...
  typeSelect.innerHTML = QUESTION_TYPES.map(type => 
...
async function loadOptions() {
  try {
    ...
    const jobSelect = document.getElementById("job-select");
    if (jobSelect) {
      jobSelect.innerHTML = `<option value="">Failed to load job positions</option>`;
    }
  }

  try {
    const candidates = await api.getCandidates();
    console.log("Candidates received from backend:", candidates);
    ...
    const candSelect = document.getElementById("candidate-select");
    if (candSelect) {
      candSelect.innerHTML = `<option value="">Failed to load candidates</option>`;
    }
  }
}
...
async function generateQuestions() {
  ...
  const questionType = document.getElementById("type-select")?.value || "technical";
  ...
  container.innerHTML = `<p style="font-size:12px; color:var(--text-muted); padding:12px;">⚡ Generating ${typeLabel} questions...</p>`;
  ...
  try {
    ...
    const fallbackQuestions = getFallbackQuestions(questionType, jobTitle);
    
    // Try to use Google Studio API if configured
    let questions = [];
    try {
      const response = await fetch('YOUR_GOOGLE_STUDIO_API_ENDPOINT', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          prompt: `Generate 5 ${questionType} interview questions for a ${jobTitle} position.`,
          temperature: 0.7,
          max_tokens: 500
        })
      });
    ...
    // Clean up question display - remove extra info
    container.innerHTML = questions.map((q, idx) => {
      const qText = q.question || q.text || q.content || "Interview question";
      const cleanQuestion = qText.replace(/\[.*?\]/g, '').replace(/\(.*?\)/g, '').trim();
      
      return `
        <div class="question-card">
          <div class="q-number">${idx + 1}</div>
          <div class="q-body">
            <div class="q-text">${escapeHtml(cleanQuestion)}</div>
          </div>
        </div>
      `;
    }).join("");
    ...
    const jobTitle = document.getElementById("job-select")?.options?.[document.getElementById("job-select").selectedIndex]?.text || "Software Developer";
    const fallbackQuestions = getFallbackQuestions(questionType, jobTitle);
    
    container.innerHTML = fallbackQuestions.map((q, idx) => {
      const qText = q.question || q.text || q.content || "Interview question";
      const cleanQuestion = qText.replace(/\[.*?\]/g, '').replace(/\(.*?\)/g, '').trim();
...
// ============================================================
// Fallback Questions by Type (SIMPLIFIED - 5 types only)
// ============================================================
function getFallbackQuestions(type, jobTitle) {
  const questions = {
    technical: [
      ...
      { question: "What is your approach to debugging complex technical issues?" },
      { question: "How do you stay updated with new technologies and frameworks?" },
      { question: "Describe a technical challenge you faced and how you solved it." }
    ],
    behavioral: [
      { question: `Describe a time you had to work under pressure as a ${jobTitle}. How did you handle it?` },
      { question: "Tell me about a conflict you had with a team member and how you resolved it." },
      { question: "Describe a situation where you went above and beyond for a project." },
      { question: "How do you handle constructive criticism and feedback?" },
      { question: "Tell me about a time you failed and what you learned from it." }
    ],
    scenario: [
      { question: `You are working on a ${jobTitle} project and the deadline is approaching. How do you prioritize tasks?` },
      { question: "A critical bug is found in production. Describe your approach to fixing it." },
      { question: "Your team disagrees on a technical approach. How do you handle the situation?" },
      { question: "A stakeholder requests a new feature mid-sprint. What do you do?" },
      { question: "You discover a security vulnerability in your application. What steps do you take?" }
    ],
    aptitude: [
      { question: "If you have 8 balls and one is heavier than the rest, how do you find the heavier ball in 2 weighings?" },
      { question: "What is the next number in the sequence: 2, 6, 12, 20, 30, ___?" },
      { question: "A train travels at 60 km/h and covers a distance of 180 km. How long does it take?" },
      { question: "If 5 people can paint 5 houses in 5 days, how long will it take 10 people to paint 10 houses?" },
...
function startInterview() {
  ...
  finalInterviewScore = 0;
  chatHistory = [];
  currentQuestion = "Could you briefly introduce your technical background?";
  questionNumber = 1;
  totalQuestions = 5;
  answerScores = [];
  ...
  appendMsg("ai", `Hello ${activeCandidateName}, I'm your AI interviewer today. Let's begin the interview simulation.\n\n📝 ${currentQuestion}`);
}
...
function endInterview() {
  ...
  if (answerScores.length > 0) {
    finalInterviewScore = answerScores.reduce((sum, score) => sum + score, 0) / answerScores.length;
    displayFinalScoreToCandidate(finalInterviewScore);
    displayInterviewerRecommendation(finalInterviewScore);
  } else {
...
function displayFinalScoreToCandidate(finalScore) {
  let scoreMessage = `🏁 **Interview Completed!**\n\n`;
  scoreMessage += `📊 **Final Score: ${Math.round(finalScore)}/100**\n`;
  scoreMessage += `📝 Questions Answered: ${answerScores.length}/${totalQuestions}\n\n`;
  
  if (answerScores.length > 0) {
    scoreMessage += `**Score Breakdown:**\n`;
    answerScores.forEach((score, index) => {
      const questionText = chatHistory[index]?.question || `Question ${index + 1}`;
      const shortQuestion = questionText.length > 50 ? questionText.substring(0, 50) + "..." : questionText;
      let emoji = score >= 70 ? "✅" : score >= 40 ? "⚠️" : "❌";
      scoreMessage += `${emoji} Q${index + 1}: ${Math.round(score)}/100 - ${shortQuestion}\n`;
    });
  }
  
  scoreMessage += `\n📌 Your interview has been completed. The hiring team will review your responses and get back to you shortly.`;

  appendMsg("ai", scoreMessage);
}
...
function displayInterviewerRecommendation(finalScore) {
  const recommendation = getAIRecommendation(finalScore);
  ...
  highlightRecommendedButton(recommendation.status);
}
...
function highlightRecommendedButton(recommendedStatus) {
  ...
  if (recommendedBtn) {
    ...
    recommendedBtn.style.border = "3px solid #3b82f6";
    recommendedBtn.style.transform = "scale(1.05)";
    recommendedBtn.style.boxShadow = "0 4px 12px rgba(59, 130, 246, 0.3)";
  }
}
...
function displayCurrentScore() {
  if (answerScores.length === 0) {
    appendMsg("ai", "📊 You haven't answered any questions yet. Please provide answers to get scored.");
    return;
  }
  
  const currentAverage = answerScores.reduce((sum, score) => sum + score, 0) / answerScores.length;
  let scoreMessage = `📊 **Current Progress Score: ${Math.round(currentAverage)}/100**\n`;
  scoreMessage += `📝 Questions Answered: ${answerScores.length}/${totalQuestions}\n\n`;
  
  if (answerScores.length > 0) {
    scoreMessage += `**Your scores so far:**\n`;
    answerScores.forEach((score, index) => {
      const questionText = chatHistory[index]?.question || `Question ${index + 1}`;
      const shortQuestion = questionText.length > 40 ? questionText.substring(0, 40) + "..." : questionText;
      let emoji = score >= 70 ? "✅" : score >= 40 ? "⚠️" : "❌";
      scoreMessage += `${emoji} Q${index + 1}: ${Math.round(score)}/100\n`;
    });
  }
  
  let rating = "";
  if (currentAverage >= 80) rating = "🌟 Excellent performance so far! Keep it up!";
  else if (currentAverage >= 60) rating = "👍 Good performance. Keep going!";
  else if (currentAverage >= 40) rating = "📚 Average performance. Try to provide more technical details in your answers.";
  else rating = "📖 Your answers need more technical depth. Focus on explaining your thought process clearly.";
...
async function sendChatMessage() {
  ...
  if (!input || !input.value.trim() || !isInterviewActive || !activeCandidateId) {
    return;
  }
  ...
  const validation = validateTechnicalAnswer(userText, currentQuestion);
  
  if (!validation.valid) {
    const example = getExampleAnswer(currentQuestion);
    appendMsg("ai", `${validation.reason}${example}\n\n📝 ${currentQuestion}`);
    input.disabled = false;
    document.getElementById("send-msg-btn")?.removeAttribute("disabled");
    input.focus();
    return;
  ...
  // Calculate score
  const frontendScore = calculateAnswerScore(userText, currentQuestion);
  ...
  const answeredQuestionNumber = questionNumber;

  try {
    const simFn = api.simulateInterviewTurn || api.simulateInterviewChat;

    const data = await simFn(
      activeCandidateId,
      userText,
      chatHistory,
      answeredQuestion,
    ...
    let reply = data?.ai_response || data?.reply || "Please provide more details about your answer.";

    let finalAnswerScore = frontendScore;
    if (data?.answer_score && data.answer_score > 0) {
      finalAnswerScore = data.answer_score;
    }
    ...
    answerScores.push(finalAnswerScore);

    if (data?.needs_retry === true || data?.is_valid === false) {
      currentQuestion = data?.current_question || answeredQuestion;
      console.log("Invalid answer. Same question:", currentQuestion);
      ...
      let newQuestion = data?.next_question?.trim();

      if (!newQuestion || newQuestion.toLowerCase() === answeredQuestion.toLowerCase()) {
        ...
        newQuestion = fallbackQuestions[(questionNumber - 2) % fallbackQuestions.length];
      }

      currentQuestion = newQuestion;
      console.log("NEW QUESTION:", currentQuestion);

      const aiReplyHasQuestion = reply.includes(currentQuestion) || 
                                 reply.includes('📝') ||
                                 reply.toLowerCase().includes('next question');

      if (!aiReplyHasQuestion && questionNumber <= totalQuestions) {
        if (reply.includes('move to the next question') && !reply.includes(currentQuestion)) {
          appendMsg("ai", reply);
          appendMsg("ai", `📝 ${currentQuestion}`);
        } else {
          appendMsg("ai", reply);
          appendMsg("ai", `📝 ${currentQuestion}`);
        }
      ...
      if (questionNumber > totalQuestions) {
        ...
        const sessionBadge = document.getElementById("session-badge");
        if (sessionBadge) {
          sessionBadge.textContent = "Interview Completed";
          sessionBadge.style.backgroundColor = "var(--bg-subtle)";
          sessionBadge.style.color = "var(--text-muted)";
        }
    ...
    appendMsg("ai", "I couldn't process that response. Please try answering the current question again.");
  } finally {
...
function renderAtsCards(candidates) {
  ...
  container.innerHTML = candidates.slice(0, 2).map(c => {
    ...
    const statusText = c.status || "In review";
    let statusColor = "#6b7280";
    if (statusText.toLowerCase() === 'shortlisted') statusColor = "#22c55e";
    else if (statusText.toLowerCase() === 'scheduled') statusColor = "#3b82f6";
    else if (statusText.toLowerCase() === 'rejected') statusColor = "#ef4444";
...
async function updateCandidatePipelineStatus(newStatus) {
  ...
  let confirmMessage = `Are you sure you want to mark ${activeCandidateName} as "${newStatus}"?`;
  
  if (interviewCompleted && finalInterviewScore > 0) {
    const recommendation = getAIRecommendation(finalInterviewScore);
  ...
  try {
    ...
    updateAtsButtons(newStatus);
    alert(`✅ Candidate ${activeCandidateName} has been ${newStatus}!`);
  } catch (err) {
